...
- `npm run build:libs` + build từng app (`apps/api`, `apps/worker`, `frontend` qua `tsc --noEmit`) phải sạch.
- `libs/failover` + `libs/shared` jest suite phải pass.
...
## Docker local
`docker compose up -d` từ root. Cấu hình server preview trong `.claude/launch.json` (api :3001, worker :3099, frontend :3000, stack toàn bộ qua `docker compose up`).
