...
1. **Xác định `channelType`**: adapter mới thuộc `channel_type` nào đã có (`zbs`, `sms`, `telegram`, `line`, `whatsapp`, `email`) hay cần thêm type hoàn toàn mới (thêm vào `ChannelType` enum ở `libs/domain/src/enums`).

2. **Viết class implement `ChannelAdapter`** (`libs/adapters/src/channel-adapter.interface.ts`) tại `libs/adapters/src/<provider>/<provider>.adapter.ts`:
...
3. **Đăng ký** trong `libs/adapters/src/adapters.module.ts` (provider + push vào danh sách registry).

4. **Nếu cần config UI đặc thù**: kiểm tra `getConfigSchema()` đã đủ để frontend render form động chưa (frontend/app/channels đọc schema, không cần sửa gì thêm nếu field type chuẩn).

5. **Nếu có webhook delivery status thật**: thêm controller ở `apps/api/src/modules/webhooks/` theo pattern channel-scoped (`/webhooks/<type>/:channelId`) trừ khi provider tự phân biệt channel qua payload (như WhatsApp dùng `phone_number_id`) thì dùng 1 URL chung — xem design.md mục "Webhook URL theo từng channel".

6. **Nếu provider yêu cầu opt-in trước khi gửi được** (giống Telegram/Zalo/LINE): xem pattern capture ở `apps/api/src/modules/webhooks/telegram-webhook.controller.ts` + `contacts.service.ts#upsertIdentifier` — không tự chế cơ chế mới nếu pattern có sẵn áp dụng được.

7. **Build + test**: `npm run build:libs`, rebuild `apps/api`/`apps/worker` nếu cần, đảm bảo `libs/failover` + `libs/shared` test vẫn pass.
